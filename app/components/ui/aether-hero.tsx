...
export default function AetherHero({
  ...
  return (
    ...
    >
      ...
      {(title || subtitle || ctaLabel || secondaryCtaLabel) && (
        <div
        ...
        >
          <div style={{ width: '100%', maxWidth, textAlign }}>
            {title && <h1 style={{ margin: 0, fontSize: 'clamp(2.2rem, 6vw, 4.5rem)', fontWeight: 700 }}>{title}</h1>}
            {subtitle && <p style={{ marginTop: '1rem', fontSize: 'clamp(1rem, 2vw, 1.25rem)', opacity: 0.9 }}>{subtitle}</p>}
          </div>
        </div>
      )}
    </section>
